...
if __name__ == "__main__":
    ...
    tf_slr = get_slr_time_feats(events)
    ...
    lstgs = pd.DataFrame(index=idx).join(L.drop('flag', axis=1)).join(
        tf_slr.rename(lambda x: 'tf_' + x, axis=1))   
    ...
    # save separately
    filename = lambda x: prefix + '_' + x + '.pkl'
