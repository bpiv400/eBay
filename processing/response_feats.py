...
def add_norm_val(offrs, threads):
    ...
    for turn in range(1, 8):
        ...
        if turn == 1:
            start_price = threads.loc[curr_threads, 'start_price_usd'].values
            offrs.loc[curr_offr_inds, 'norm_price'] = offrs.loc[curr_offr_inds,
                                                                'offr_price'].values / start_price
        else:
            ...
            if turn == 2:
                old_offrs = threads.loc[curr_threads, 'start_price_usd'].values
            # otherwise set the old offers to 2 turn sago
            else:
                old_offr_inds = [(ind, turn - 2)
                                 for ind in curr_threads.values]
                old_offrs = offrs.loc[old_offr_inds, 'offr_price'].values
            # compute normalized value
            curr_offrs = offrs.loc[curr_offr_inds, 'offr_price'].value
...
def add_message_val(df):
    ...
    for turn in range(1, 7):
        ...
        next_offrs = df.loc[next_offr_inds, 'offr_price'].values
