...
class Simulator(nn.Module):
    ...
    def forward(self, x_fixed, x_offer):
        ...
        theta, _ = nn.utils.rnn.pad_packed_sequence(theta, total_length=MAX_TURNS)

        # output layer w/exponential transform: (seq_len, batch_size, N_output)
        return torch.exp(self.output(theta))


class DelayLoss(Function):
    '''
    Computes the negative log-likelihood for the slr delay model.

    Inputs to forward:
        - theta (N_turns, mbsize, 3): [gamma_exp, alpha, beta]
        - y (N_turns, mbsize): slr delay (1 == 2 days)
    ...
    def forward(ctx, theta, y):
        ...
        ll = torch.sum(torch.log(1 - p[idx[3]]) + lnp[idx[3]])
        ll += torch.sum(torch.log(p[idx[2]]))
    ...
    def backward(ctx, grad_output):
        ...
        dg = idx[2] * torch.pow(ctx.g, -1) - torch.pow(1 + ctx.g, -1)
