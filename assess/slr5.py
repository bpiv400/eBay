import numpy as np
from assess.util import ll_wrapper, add_byr_reject_on_listing_expiration
...
def main():
    ...
    norm5 = (norm * ~data[X_OFFER][REJECT]).xs(5, level=INDEX).reindex(
        index=con5.index, fill_value=0)
    ...
    x1, x2 = norm3.values, con4.values
    y_acc, y_rej, y_norm = (con5 == 1).values, (con5 == 0).values, norm3.values

    for feat in ['acc', 'rej', 'norm']:
        key = 'response_rejrej{}'.format(feat)
